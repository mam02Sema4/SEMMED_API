...
# Start Chunking
#for chunk in pd.read_sql(query, con=db_connection,chunksize=100000):
...
#df = pd.read_sql('SELECT SUBJECT_CUI,PREDICATE,OBJECT_CUI,PMID FROM pubmed.PREDICATION', con=db_connection)
#dfs.to_csv('result1.csv', index=False)
...
df_iter = pd.read_csv('result1.csv', dtype=str, chunksize=10000000)
